export class Md2 {
  ...
  constructor(data: Uint8Array) {
    ...
    // 3. message + checksum
    const finalData = new Uint8Array(padded.length + 16);

    finalData.set(padded);
    finalData.set(checksum, padded.length);

    // 4. Main transform
    const X = new Uint8Array(48);

    for (let offset = 0; offset < finalData.length; offset += 16) {
      // copy block
      for (let j = 0; j < 16; j++) {
        X[16 + j] = finalData[offset + j];
